"""iTerm2 continuous scroll - sends wheel-tick-sized events at readable pace."""
...
def do_one_scroll():
    """Send one smooth scroll increment. Stops if iTerm2 loses focus."""
    from talon import ui

    # Stop scrolling if we've left iTerm2
    active_app = ui.active_app()
    if active_app.bundle != "com.googlecode.iterm2":
        actions.user.iterm2_scroll_stop()
        return
...
class Actions:
    ...
    def iterm2_scroll_start_down():
        """Start scrolling down continuously"""
        global scroll_job, scroll_direction, saved_mouse_pos, scroll_interval_ms

        actions.app.notify("Starting scroll down")
